...
client.on('guildMemberAdd', (member) => {
    ...
    member.guild.fetchInvites().then(async guildInvites => {
        ...
        let şüphe = Date.now() - member.user.createdTimestamp < 1000 * 60 * 60 * 24 * 10 ? ayar.emojis.no : ayar.emojis.yes
        if (şüphe === ayar.emojis.no) {
            ...
            member.guild.channels.cache.get(ayar.channel.registerChat).send(
                ...
                .setDescription(`${member}, Adlı kullanıcı sunucuya katıldı fakat hesabı **${createAt2}** önce açıldığı için şüpheli!`)
                .setTimestamp()
                .setFooter(`Bu kullanıcı sunucumuza ${y} tarafından davet edilmişti`))
...
Hesabın ${moment(member.user.createdTimestamp).locale('tr').format("LLL")} tarihinde (\`${moment.duration(Date.now() - member.user.createdTimestamp).format('Y [yıl], M [Ay], D [Gün]')}\`) önce oluşturulmuş.
        
Sunucu kurallarımız <#${ayar.channel.rules}> kanalında belirtilmiştir. Unutma sunucu içerisindeki ceza-i işlemlerin kuralları okuduğun varsayılarak gerçekleştirilecek!
        
Ayrıca bize destek olmak için tagımızı alabilirsin, tagımız için ise herhangi bir kanala **.tag** yazabilirsin.

${y}  ${davetsayi}. Davetini gerçekleştirerek sunucumuzun __${member.guild.memberCount}.__ üyesi olmanı sağladı :tada::tada:
`)
        }
    });
});
